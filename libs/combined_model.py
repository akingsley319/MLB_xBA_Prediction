# -*- coding: utf-8 -*-
"""
Created on Mon Feb 27 01:23:49 2023
...
def combined_prep(df,df_batter,df_pitcher):
    ...
    x_matchup["estimated_ba_using_speedangle"] = y_matchup
    x_matchup['pa'] = pa_data
    x_matchup = x_matchup.join(y_batter['batter_pred'],on=['batter','game_date'])
    x_matchup = x_matchup.join(y_pitcher['pitcher_pred'],on=['pitcher','game_date'])
    x_matchup.dropna(inplace=True)
    # separates data
    X = x_matchup.loc[:,~x_matchup.columns.isin(['estimated_ba_using_speedangle','pa'])]
    y = x_matchup.loc[:,x_matchup.columns.isin(['estimated_ba_using_speedangle'])]
    pa_data =  x_matchup.loc[:,x_matchup.columns.isin(['pa'])]
    return X, y, pa_data

def combined_perf(x_train,y_train,param_grid=None,intense=False,save=False):
    combined_model = Combined()
    
    if intense == True:
        combined_model.fit_rf_intense(x_train,y_train,param_grid=param_grid,replace=True)
    elif intense == False:
        combined_model.fit_rf(x_train,y_train,replace=True)
        
    if save == True:
        combined_model.save_model()
        
    return combined_model.model
